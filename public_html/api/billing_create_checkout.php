...
function billing_log_checkout(string $message): void
{
    $logDir = __DIR__ . '/../logs';
    if (!is_dir($logDir)) {
...
try {
    ...
    billing_log_checkout("Iniciando checkout para Loja ID: {$lojaId}");
    billing_log_checkout('Payload enviado: ' . json_encode($payload, JSON_UNESCAPED_UNICODE));
    ...
    if ($response === false) {
        billing_log_checkout('Erro cURL: ' . $curlError);
        throw new Exception('Erro ao conectar ao Mercado Pago: ' . $curlError);
    }

    billing_log_checkout('Resposta MP (' . $httpCode . '): ' . $response);
    ...
    if ($httpCode >= 400) {
        ...
        billing_log_checkout('Falha no checkout: ' . $message);
    ...
    if (empty($data['id'])) {
        billing_log_checkout('Resposta inválida do MP: ' . $response);
    ...
    billing_log_checkout('Erro crítico: ' . $e->getMessage());
